...
Verify threat metadata within the NoSQL HBase cluster.
* **Browse Table**: `python -m src.utils.browse_hbase`
  * *Action*: View the top 25 records currently residing in the Serving Layer.
* **Targeted Metadata Lookup**: `python -m src.utils.query_intel <IP_OR_HASH>`
  * *Example*: `python -m src.utils.query_intel 1.12.76.172`
* **Behavioral Search**: `python -m src.utils.search_threat "Brute-Force"`
  * *Action*: Search the distributed cluster by attack type or keyword.

#### 2. View Detection Hits (Speed Layer)
Display real-time alerts generated by the Spark streaming engine (the "Speed Layer").
* **Alert Summary**: `python -m src.utils.view_alerts`
  * *Action*: Provides a human-readable summary of traffic matches detected in the current micro-batch.

#### 3. Forensic Investigation (Batch Layer)
Perform deep-dive "Cross-Layer" searches across the 1.5 GB historical log repository in HDFS.
* **Historical Trace**: `python -m src.utils.cross_layer_search <IP_ADDRESS>`
  * *Example*: `python -m src.utils.cross_layer_search 10.250.19.102`
  * *Action*: Scans the Batch Layer to correlate a real-time alert with historical system-wide interactions.
...
## 3. HDFS System Logs (Batch Layer)
